{{-- ============================================================
     FILE   : resources/views/koordinator/dashboardkoor.blade.php
     LAYOUT : layouts.koordinator
     ============================================================ --}}
@extends('layouts.koordinator')
@section('title', 'Dashboard Koordinator')
@section('breadcrumb')
    <span>PST</span><span>›</span><strong>Dashboard Koordinator</strong>
@endsection

@push('styles')
<style>
/* ══════════════════════════════════════
   DASHBOARD — sama persis struktur admin
   ══════════════════════════════════════ */
...
.db-badge {
    ...
    background:var(--green-lt); border:1px solid rgba(10,124,78,.15);
...
.kpi-deco   { position:absolute; right:16px; top:14px; opacity:.18; }
...
/* ════════════ ROW 3 — sama persis admin ════════════ */
.row-3 {
    display:grid; grid-template-columns:repeat(3,1fr);
    gap:12px; align-items:stretch;
}
.row-3 > .panel { display:flex; flex-direction:column; }
.row-3 > .panel .jdw-list,
.row-3 > .panel .act-list { flex:1; }
...
/* ════════════ ROW 2-1 (bawah) ════════════ */
.row-2-1 { display:grid; grid-template-columns:1fr 340px; gap:12px; align-items:start; }
...
.abs-center { position:absolute; inset:0; display:flex; flex-direction:column; align-items:center; justify-content:center; pointer-events:none; }
...
.abs-grid  { display:grid; grid-template-columns:repeat(4,1fr); gap:1px; background:var(--rule); border-top:1px solid var(--rule); }
...
/* ════════════ JADWAL ════════════ */
.jdw-row { display:flex; align-items:center; gap:10px; padding:9px 16px; border-bottom:1px solid var(--rule); }
...
.td-id   { font-size:10px; color:var(--ink3); margin-top:1px; font-family:'IBM Plex Mono',monospace; }
.mava { display:inline-flex; align-items:center; justify-content:center; width:22px; height:22px; border-radius:3px; background:var(--wash2); font-size:9px; font-weight:700; color:var(--ink2); margin-right:6px; text-transform:uppercase; }
...
.btn-review { font-size:11px; font-weight:500; padding:4px 10px; border-radius:4px; border:1px solid var(--rule); background:var(--wash); color:var(--ink2); cursor:pointer; transition:all .12s; font-family:'IBM Plex Sans',sans-serif; }
.btn-review:hover { border-color:var(--green); background:var(--green-lt); color:var(--green); }
...
.qa { display:flex; align-items:center; gap:8px; padding:9px 10px; border:1px solid var(--rule); border-radius:6px; background:var(--wash); text-decoration:none; transition:border-color .12s, background .12s; }
...
/* ════════════ EMPTY STATES ════════════ */
.db-empty { padding:28px 16px; text-align:center; color:var(--ink3); font-size:12px; }
...
<div class="db-head">
    <div>
        <h1>Monitoring Operasional — {{ $wilayah->nama ?? 'Wilayah Saya' }}</h1>
        <p>{{ \Carbon\Carbon::now('Asia/Jakarta')->translatedFormat('l, d F Y') }} &middot; Pantau kinerja dan aktivitas tim petugas wilayah Anda</p>
    </div>
...
<div>
    ...
    <div class="kpi-row">
        ...
        {{-- Total Petugas --}}
        <a href="{{ route('koordinator.tim-petugas') }}" class="kpi kpi-blue">
            <div class="kpi-deco">
            ...
            <div class="kpi-label">Petugas Wilayah</div>
        ...
        {{-- Hadir Hari Ini --}}
        <a href="{{ route('koordinator.absensi.index') }}" class="kpi kpi-green">
            <div class="kpi-deco">
        ...
        {{-- Laporan Pending --}}
        <a href="{{ route('koordinator.laporan.harian.index') }}" class="kpi kpi-amber">
            <div class="kpi-deco">
        ...
        {{-- Rata-rata Kinerja --}}
        <a href="{{ route('koordinator.nilai-evaluasi.index') }}" class="kpi kpi-purple">
            <div class="kpi-deco">
...
══════════════════════════════════════ --}}
<div>
    <div class="sec">Tren Aktivitas Tim</div>
    <div class="panel">
        <div class="ph">
            <div>
                <div class="ph-title">Grafik Aktivitas — {{ $wilayah->nama ?? 'Wilayah Saya' }}</div>
                <div class="ph-sub" id="chart-sub-desc">30 hari terakhir — setiap titik = 1 hari</div>
            </div>
            <div class="chart-tabs">
                <button class="ctab active" onclick="switchChart('perPetugas',this)">Per Petugas</button>
        ...
        {{-- Ringkasan angka - 1 baris compact --}}
        ...
        <div style="background:#f8faff;border-bottom:1px solid var(--rule);padding:8px 16px;display:flex;align-items:center;gap:8px;flex-wrap:wrap">
            ...
            <span style="font-size:10.5px;color:#374151"><strong>Cara baca:</strong> Garis <span style="color:#1a56db;font-weight:600">biru</span> = absensi masuk petugas wilayah ini, <span style="color:#0a7c4e;font-weight:600">hijau</span> = laporan shift dikirim, <span style="color:#b45309;font-weight:600">oranye</span> = checklist selesai. Hover titik untuk detail angka.</span>
        </div>
    ...
    <div class="row-3">
        ...
        <div class="panel">
            <div class="ph">
                <div>
                    ...
                    <div class="ph-sub">Status kehadiran petugas {{ $wilayah->nama ?? '' }}</div>
                </div>
                <a href="{{ route('koordinator.absensi.index') }}" class="ph-link">Detail →</a>
            </div>
            <div class="abs-wrap">
                <div class="abs-donut">
            ...
            <div class="abs-grid">
                <div class="abs-cell">
                    <div class="abs-cnum" style="color:#0a7c4e">{{ $absensiStatus['tepat_waktu'] ?? 0 }}</div>
        ...
        <div class="panel">
            <div class="ph">
                ...
                <a href="{{ route('jadwal.index') }}" class="ph-link">Kelola →</a>
            </div>
            ...
            <div class="jdw-row">
                ...
                <div style="flex:1;min-width:0">
                    ...
                    <div class="jdw-wil">{{ ucfirst($j->shift ?? '-') }}</div>
                </div>
            </div>
            @empty
            <div class="db-empty">
...
{{-- ══════════════════════════════════════
     LAPORAN PENDING · PERFORMA TIM · AKSI CEPAT
══════════════════════════════════════ --}}
<div>
    <div class="sec">Laporan &amp; Kinerja Tim</div>
    <div class="row-2-1">

        {{-- Kiri: Laporan Pending + Performa Tim --}}
        <div style="display:flex;flex-direction:column;gap:12px">
            ...
            <div class="panel">
                <div class="ph">
                    <div>
                        ...
                        <div class="ph-sub">{{ $laporanPending ?? 0 }} laporan menunggu review Anda</div>
                    </div>
                    <a href="{{ route('koordinator.laporan.harian.index') }}" class="ph-link">Lihat semua →</a>
                </div>
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        ...
                        <tr onclick="window.location='{{ route('koordinator.laporan.harian.index') }}'" style="cursor:pointer">
                            <td>
                            ...
                            <td class="mono">{{ \Carbon\Carbon::parse($l->updated_at)->format('d M H:i') }}</td>
                        ...
                        <tr>
                            <td colspan="4">
                                <div class="db-empty" style="padding:20px 0">Tidak ada laporan pending</div>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="panel">
                <div class="ph">
                    <div>
                        <div class="ph-title">Performa Tim</div>
                        <div class="ph-sub">Skor evaluasi terakhir per petugas</div>
                    </div>
                    <a href="{{ route('koordinator.nilai-evaluasi.index') }}" class="ph-link">Detail →</a>
                </div>
                @forelse($performaTim ?? [] as $pt)
                <div class="prog-row">
                    <div class="prog-label">{{ $pt->nama }}</div>
                    <div class="prog-track">
                        <div class="prog-fill" style="width:{{ $pt->nilai }}%;background:{{ $pt->nilai >= 90 ? 'var(--green)' : ($pt->nilai >= 75 ? 'var(--blue)' : 'var(--amber)') }}"></div>
                    </div>
                    <div class="prog-val">{{ round($pt->nilai) }}</div>
                </div>
                @empty
        ...
        <div>
            <div class="panel">
                ...
                <div class="qa-grid">
                    <a href="{{ route('koordinator.absensi.index') }}" class="qa">
                    ...
                    <a href="{{ route('jadwal.index') }}" class="qa">
                    ...
                    <a href="{{ route('koordinator.laporan.harian.index') }}" class="qa">
                    ...
                    <a href="{{ route('koordinator.checklist.index') }}" class="qa">
                    ...
                    <a href="{{ route('koordinator.nilai-evaluasi.index') }}" class="qa">
                        <svg width="13" height="13" fill="none" stroke="var(--ink3)" stroke-width="1.5" viewBox="0 0 24 24"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
                        <span class="qa-lab">Penilaian</span>
                    </a>
                    <a href="{{ route('koordinator.tim-petugas') }}" class="qa">
...
const CHART_DESC = {
    perPetugas: 'Perbandingan per anggota tim — bulan berjalan',
...
function buildChart(mode) {
    ...
    const isPetugas = mode === 'perPetugas';
    ...
    const ds = [
        ...
        { label:'Rata Kinerja',      data:kinerja,   type:'line', yAxisID:'y2', borderColor:'#dc2626', backgroundColor:'transparent',
          ...
          pointRadius:isPetugas?5:3, pointHoverRadius:7, pointBackgroundColor:'#fff', pointBorderColor:'#dc2626', pointBorderWidth:2 },
    ];
        ...
        options: {
                ...
                tooltip: {
                    ...
                    callbacks: {
                        label: item => {
                            ...
                            const icons = { 'Kehadiran':'👥', 'Laporan Masuk':'📄', 'Checklist Selesai':'✅', 'Rata Kinerja':'📊' };
            ...
            scales: {
                x: {
                    grid: { display:false },
                    ticks: {
                        font:{ size: isPetugas ? 11 : 10, family:'IBM Plex Mono' },
                        color:'#6b7280', maxRotation:0, maxTicksLimit:20
                    }
                },
                y: {
                    position:'left',
